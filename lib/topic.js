...
const loginStatus = (request, response) => {
    ...
    if(request.headers.cookie === undefined){ // 쿠키 값이 있으면
        return authStatusUI;
    };
    
    cookies = cookie.parse(request.headers.cookie);
    if(cookies.email === 'egoing777@gmail.com' && cookies.password === `111111`){ // 맞는 아이디이면
        isOwner = true;
    }
    
    if(isOwner){ // 사용자 인증이 됐으면
        authStatusUI = `<a href = "/logout_process">logout</a>`
    }
    return authStatusUI;
};

const isMember = (request, response) => {
    let cookies = {};
    let isOwner = false;
    if(request.headers.cookie !== undefined){ // 쿠키 값이 있으면
        cookies = cookie.parse(request.headers.cookie);
    ...
    connection.query(`select * from topic`, (error, topics) => {
        ...
        connection.query(`select topic.id, title, description, name from topic left join author on author.id = topic.author_id where topic.id=?`,
            ...
            (error2, topic) => {
                ...
                </form>`,loginStatus(request, response));
                response.send(templates);
            })
    })
};
    ...
    connection.query(`select * from topic`, (err, topics) => {
        ...
        connection.query(`select * from author`, (err2, authors) => {
            ...
            let templates = template.HTML(title, list, `
                ...
                `, `<a href="/create">create</a>`,loginStatus(request, response));
            response.send(templates);
        });
    });
};
    ...
    connection.query(`select * from topic`, (err, topics) => {
        ...
        connection.query(`select * from topic where id = ?`, [request.params.update_id], (err2, topic) => {
            ...
            connection.query(`select * from author`, (err3, authors) => {
                ...
                `,
                    `<a href="/create">create</a> <a href="/update?id=${topic[0].id}">update</a>`,loginStatus(request, response));
                response.send(templates);
            });
        });
    });
