    ...
    connection.query(`select * from users`, (err, topics) => {
        ...
        if (post.password === post.password2) {
            bcrypt.hash(post.password, 10, function (err, hash) {
                ...
                (?, ?, ?, NOW())`, [post.email, hash, post.nickname],
                    (err2, result) => {
                        if (err2) throw err2;
                        let user = {
                            email: post.email,
...
exports.create_process = (request, response) => {
    let post = request.body;
    connection.query(`select * from users where email = ?`, [request.user.email || request.user[0]. email], (err, author) => {
        if (err) throw err;
...
exports.update = (request, response) => {
    ...
    connection.query(`select * from topic where id = ?`, [request.params.update_id], (error, data) => {
        ...
        if (data[0].author_id !== (request.user.id || request.user[0].id)) {
...
exports.update_process = (request, response) => {
    let post = request.body;
    connection.query(`select * from users where email = ?`, [request.user[0].email || request.user.email], (err, author) => {
        if (err) throw err;
