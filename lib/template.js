...
module.exports = {
	HTML : (title, list, body, control, authStatusUI = `<a href = "/login">login</a> | <a href = "/register">register</a>`) => {
		return `
				<!doctype html>
				<html>
					<head>
	...
	// 		tag += `<option ${selected}>${sanitizeHtml(authors[i].email)}</option>`
	// 	}
	// 	return tag;
	// },
	authorList : (authors) => {
		...
		for(let i = 0; i < authors.length; i++){
			table += '<tr>';
			table += `<td>${sanitizeHtml(authors[i].email)}</td>`
			table += `<td>${sanitizeHtml(authors[i].nickname)}</td>`
